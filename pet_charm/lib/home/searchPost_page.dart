...
class _SearchPostPage extends State<SearchPostPage> {
  ...
  Widget itemBuilder(BuildContext context, int index) {
    return ListTile(
      ...
      title: Text(_models[index].title),
      subtitle: Container(
        alignment: Alignment.bottomLeft,
        padding: const EdgeInsets.only(right: 10),
        height: 25,
        child: Text(
          _models[index].content,
  ...
  void _onChanged(String text) {
    ...
    if (text.isNotEmpty) {
      for (int i = 0; i < _posts.length; i++) {
        String? name = _posts[i].content;
        String? title = _posts[i].title;
        if ((name).contains(text) || (title).contains(text)) {
          _models.add(_posts[i]);
        }
      }
    }
    setState(() {
