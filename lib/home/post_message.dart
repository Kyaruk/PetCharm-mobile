...
class PostMessage extends StatefulWidget {
  ...
  @override
  _PostMessage createState() => _PostMessage();
}

class _PostMessage extends State<PostMessage> {
    ...
    try {
      ...
      if (imagePath.path == "") {
        ...
        formData = FormData.fromMap({
          "postTitle": _postTitleController.text,
          "postContent": _postController.text,
          "postCover": await MultipartFile.fromFile(path, filename: name)
        });
      }

      // 多个文件上传
      // var response = await Dio().post('10.0.0.2:5000/post/', data: formData);
      var response = await Global.dio.post('post/', data: formData);
...
//     {
//       "1": await MultipartFile.fromFile(path,
...
//   try {
//     var respone =
//         await dio.post("http://10.0.2.2:5000/test", data: formdata);
